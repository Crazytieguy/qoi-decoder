...
fn skip_two_bits<'a, O>(
    parser: impl Parser<(&'a [u8], usize), O, nom::error::Error<(&'a [u8], usize)>>,
) -> impl FnMut(&'a [u8]) -> IResult<&[u8], O> {
    bits(preceded::<_, u8, _, _, _, _>(take(2_usize), parser))
}
